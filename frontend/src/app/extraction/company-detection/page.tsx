...
export default function CompanyDetectionPage() {
  ...
  // Alias inline add
  const [addingForCompany, setAddingForCompany] = useState<string | null>(null);
  ...
  const [newCompanyLogoAssetId, setNewCompanyLogoAssetId] = useState("");
  ...
  const load = useCallback(async (showSpinner = false) => {
    ...
    setError("");
    try {
      const [companyResult, aliasResult, logoResult] = await Promise.all([
      ...
      setOriginalCompanies(companyResult.companies.items);
      setOriginalAliases(aliasResult.aliases.items);
      setCompanies(companyResult.companies.items);
      setAliases(aliasResult.aliases.items);
      setDeletedCompanyIds(new Set());
      setLogos(logoResult.assets.items);
    } catch (reason) {
  ...
  const hasChanges = useMemo(() => {
    ...
    for (const c of companies) {
      ...
      if (!orig) return true;
      if (orig.status !== c.status || orig.name !== c.name || orig.logo_asset_id !== c.logo_asset_id) {
        return true;
      }
    }
  ...
  const filteredCompanies = useMemo(() => {
    ...
    return companies
      ...
      .filter((c) => {
        ...
        const companyAliases = aliasesByCompany.get(c.id) || [];
        return companyAliases.some((a) => a.alias.toLowerCase().includes(term));
      });
  ...
  function stageQuickAddAlias(companyId: string) {
  ...
  function handleCreateCompanyFromModal() {
    ...
    const newComp: Company = {
      ...
      logo_asset_id: newCompanyLogoAssetId || null,
      logo: logos.find((l) => l.id === newCompanyLogoAssetId) || null,
    };

    setCompanies((prev) => [newComp, ...prev]);

    const initialPhrases = newCompanyAliases
      .split(/[,\n]/)
      .map((p) => p.trim())
      .filter(Boolean);
  ...
  async function handleSaveAllChanges() {
    ...
    try {
      ...
      // 2. Process company creations and updates
      ...
      <section className="grid gap-6 max-w-6xl mx-auto pb-24 relative">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="m-0 font-[var(--font-manrope)] text-[30px] font-bold text-[var(--rl-text-strong)]">
              Company Detection & Aliases
            </h1>
            <p className="mt-1 max-w-3xl text-[14px] text-[var(--rl-text-muted)]">
              Organized by insurance company. Add printed header keywords, legal names, or brand variants that resolve to each company during quotation extraction.
            </p>
          </div>
          <Button
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--rl-radius-sm)] border border-[var(--rl-border)] bg-[var(--rl-surface)] p-3 shadow-xs">
          <div className="relative flex-1 min-w-[260px] max-w-md">
            ...
            <Input
              aria-label="Search company detection phrases"
              className="pl-9 text-xs"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Filter company or detection keyword..."
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {filteredCompanies.map((company) => {
              const companyAliases = aliasesByCompany.get(company.id) || [];
              const isAdding = addingForCompany === company.id;
              ...
              return (
                <Card
                  key={company.id}
                  className={`flex flex-col justify-between p-4 border transition-all ${isActive ? "border-[var(--rl-border)] bg-white shadow-xs" : "border-dashed border-gray-300 bg-gray-50/50"}`}
                >
                  <div>
                    {/* Company Card Header */}
                      ...
                      <div className="flex items-center gap-2.5">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-md border border-[var(--rl-border)] bg-white p-1">
                          {logoAsset?.url ? (
                            ...
                            <Buildings size={18} weight="duotone" className="text-[var(--rl-text-muted)]" />
                          )}
                        </div>
                        <div>
                          <h2 className="text-sm font-bold text-[var(--rl-text-strong)]">{company.name}</h2>
                          <p className="text-[11px] text-[var(--rl-text-muted)]">{companyAliases.length} detection alias{companyAliases.length === 1 ? "" : "es"}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          type="button"
                          aria-label={`Delete insurance company ${company.name}`}
                          onClick={() => setPendingDeleteCompany(company)}
                          className="rounded-md p-1 text-[var(--rl-text-muted)] hover:bg-[var(--rl-red-light)] hover:text-[var(--rl-red)] transition-colors"
                    ...
                    {/* Alias Tag List */}
                    <div className="pt-3">
                      <p className="text-[11px] font-bold uppercase tracking-wider text-[var(--rl-text-muted)] mb-2">
                        Detection Keywords & Phrases
                      </p>
                      <div className="flex flex-wrap gap-1.5 min-h-[48px]">
                        {companyAliases.length > 0 ? (
                          companyAliases.map((item) => (
                            <span
                              key={item.id}
                          ...
                          <p className="text-xs text-[var(--rl-text-muted)] italic py-1">No custom aliases added yet.</p>
                        )}
                      </div>
                    </div>
                  </div>

                  {/* Inline Add Alias Section */}
                  <div className="mt-4 pt-3 border-t border-[var(--rl-border)]">
                    {isAdding ? (
                      <div className="flex items-center gap-2">
                        <Input
                          value={newAliasText}
                          placeholder="e.g. STMB or Takaful myMotor"
                        ...
                        <Button
                          ...
                          onClick={() => stageQuickAddAlias(company.id)}
                        >
                          Add
                        </Button>
                        <Button
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setAddingForCompany(company.id);
                          setNewAliasText("");
                        }}
      ...
      {showAddCompanyModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <Card className="w-full max-w-md border border-[var(--rl-border)] bg-white shadow-xl overflow-hidden p-6 grid gap-4">
            <div className="flex items-center justify-between border-b border-[var(--rl-border)] pb-3">
              <div>
                <h3 className="text-base font-bold text-[var(--rl-text-strong)]">Add Insurance Company</h3>
                <p className="text-xs text-[var(--rl-text-muted)]">Register a new insurer to detect in uploaded quotation PDFs.</p>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="grid gap-1.5 text-xs font-semibold text-[var(--rl-text-strong)]">
                ...
                <Select
                  ...
                  onChange={(e) => setNewCompanyStatus(e.target.value as "active" | "inactive")}
                >
              ...
              <label className="grid gap-1.5 text-xs font-semibold text-[var(--rl-text-strong)]">
                Company Logo
                <Select
                  value={newCompanyLogoAssetId}
                  onChange={(e) => setNewCompanyLogoAssetId(e.target.value)}
                >
            ...
            <label className="grid gap-1.5 text-xs font-semibold text-[var(--rl-text-strong)]">
              ...
              />
              <span className="text-[11px] text-[var(--rl-text-muted)]">Phrases or header text printed on quotation PDFs.</span>
            </label>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-[var(--rl-border)]">
              ...
              <Button
                ...
                onClick={handleCreateCompanyFromModal}
              >
                Stage Company
              </Button>
            </div>
