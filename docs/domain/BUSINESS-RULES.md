...
- Extracted Final Price (`total_amount`) is the source quotation's Total Payable / Total Contribution (which already includes base premium, taxes, stamp duty, and quotation optional covers). Insurance Premium (`premium`) is the net base insurance premium (`Final Price - Extracted Extras`). When add-ons or extras are added/removed, only the Final Price reactively updates while Insurance Premium remains stable. Road tax is automatically calculated from engine capacity and vehicle type (Malaysian JPJ rates) when absent from the quotation PDF, and is included in the Total Payable. For internal combustion engine (ICE) vehicles, capacity is measured in CC with rates based on saloon/non-saloon and ownership type. For Electric Vehicles (EV), capacity is measured in kW (inputs >= 1000 W are automatically normalized to kW); road tax follows the official JPJ 2026 ZEV rate structure (announced by MOT Malaysia / Anthony Loke, effective 1 Jan 2026) across Saloon and Non-Saloon passenger cars (sharing an identical power-band schedule from RM 20 up to RM 305 for 220 kW, RM 335 for 230 kW, etc.; 100% identical for Private and Company ownership; 50% discount for Sabah/Sarawak and Labuan > 100 kW) and Electric Motorcycles (RM 2 up to RM 42). Quotation templates display the row label as `Engine Capacity/发动机排量 : `, appending either `cc` for ICE or `kW` for EV (never both). Runner fees are populated when explicitly configured or entered by staff.
...
- Extras display titles decouple raw quotation PDF codes/descriptions from customer-facing presentation by matching to canonical Global Benefit Titles (English & Chinese) via standard benefit ID and concept code lookup. Explicit manual staff edits made during review are always preserved. Naive machine learning correction memory feedback loops are prohibited.
