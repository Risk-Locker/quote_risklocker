...
## 2026-08-31 · Antigravity (Gemini 3.1 Pro) · Fix Company Detection False Positives — EXECUTED
Asked: Fix bug where QBE was incorrectly detected as AmGen.
Done: Updated `company_alias_matches` in `backend/app/extraction/company_resolution.py` to restrict compact matches on large texts and fixed boolean sorting.
Pending: none.

## 2026-08-31 · Antigravity (Gemini 3.1 Pro) · Fix TypeScript Error with react-resizable-panels — EXECUTED
Asked: Fix IDE problems in preview-phase.tsx regarding 'onCollapse' property on Panel.
Done: Replaced 'onCollapse' and 'onExpand' with 'onResize' in `frontend/src/components/session-workspace/preview-phase.tsx` to comply with react-resizable-panels v4 API.
Pending: none.

## 2026-08-31 · Antigravity (Gemini 3.1 Pro) · Remove Extra Templates — EXECUTED
Asked: Remove "Dense A4", "Extended Portrait", and "Standard A4" templates.
Done: Soft-deleted the three specified templates from `output_template_configs` using `mark_deleted()`.
Pending: none.

## 2026-08-31 · Antigravity (Gemini 3.1 Pro) · Fix CORS for Dynamic Frontend Ports — EXECUTED
Asked: Login fails with "This request origin is not allowed." 403 Forbidden.
Done: Found that `.env` explicitly overrides `CORS_ORIGINS`. Updated `backend/app/core/config.py` to unconditionally append `3000..3010` and `8100..8110` to the allowed origins in local dev.
Pending: none.

## 2026-08-31 · Antigravity (Gemini 3.1 Pro) · Answer Dev Login Question — EXECUTED
Asked: What are the login and password?
Done: Provided the dev credentials (`admin@risklocker.local` / `admin123`) from `AGENTS.md`.
Pending: none.

## 2026-08-31 · Antigravity (Gemini 3.1 Pro) · Fix Port Selection for Frontend & Backend Scripts — EXECUTED
Asked: Make sure that the port always uses another available port if it's used by any other process (for both backend and frontend).
Done: Updated `commands/start-frontend.ps1` to loop through `3000..3010` and modified `Get-NetTCPConnection` in both scripts to correctly identify ports bound to `0.0.0.0` or `::`.
Pending: none.

## 2026-08-31 · Antigravity (Gemini 3.1 Pro) · Fix Playwright Crash & Cleanup Preview Phase UI — EXECUTED
Asked: Fix PDF extraction failing (crashing backend) and remove PNG/PDF buttons from canvas preview, plus add collapse button to left section.
Done: Removed `frozen=True` from `JobProcessingError` dataclass in `backend/app/workers/render_worker.py` and `extraction_worker.py`. Removed "Download PNG" button from `frontend/src/components/session-workspace/preview-phase.tsx` and added `react-resizable-panels` collapse button.
Pending: none.

## 2026-08-29 · Antigravity (Gemini 3.7 Flash) · Project Overview & Architecture Analysis
Asked: Provide a concise analysis and overview of the project.
Done: Analyzed repository architecture, workflows, tech stack, and subsystem modules across docs/ARCHITECTURE.md, docs/STRUCTURE.md, and docs/PROJECT-CONTEXT.md; provided structured executive summary.
Pending: none.
