...
4. **Mandatory Implementation Plan Gate**: NEVER touch, edit, or create code or configuration files without first creating an `implementation_plan.md` artifact and waiting for explicit user approval. Zero exceptions. Do not run background commands or touch code while discussing requirements with the owner.
5. **Big changes** follow the same pipeline: understand -> analyze -> research -> plan -> record plan in `docs/core/STATE.md` -> get approval -> execute -> log the outcome.
6. **Never claim success without the verification chain** in AGENTS.md (pytest, tsc, build, zero IDE/lint problems on touched files, code-map check).
7. **Update this file** whenever the owner expresses a new interaction preference — that is part of the logbook duty, not an optional chore.
8. **Check code problems/diagnostics after each change.** Always inspect and resolve IDE diagnostic problems, type errors (Pyright/TypeScript), and linter warnings on touched files immediately after making changes.
