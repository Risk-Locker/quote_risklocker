...
Benefit Packs (applied): plan levels on add-on bundles — `POST /api/business/catalogs/{id}/packages/{package_id}/plans` (create/update a plan: `name`, optional `plan_key`, `sort_order`, `status`; requires an `addon_bundle` package on the draft revision; duplicate keys 409), `DELETE /api/business/catalogs/{id}/packages/{package_id}/plans/{plan_id}` (retire, 204), `PUT /api/business/catalogs/{id}/packages/{package_id}/plans/{plan_id}/items` (replace plan members atomically: `items[]` of `{offering_id, typed_value_override?, sort_order}`; offerings must belong to the same draft revision; duplicate offerings 422; invalid typed overrides 422). Catalog publish content hash now also freezes plans + plan items. Workspace ops: `select_package_plan` (`package_id`, `plan_id`, optional `cost_status`) atomically adds/switches a plan (supersedes untouched defaults in place, preserves staff/AI custom values, drops other plans of the same bundle first); `remove_package_plan` (`plan_id`) removes members and restores superseded defaults. `create_custom_benefit` accepts an optional `price` (`{amount, currency}`) stored on the selection and surfaced as an Extras row. The workspace snapshot adds `groups` (on `benefit_cards`), `extras`, `total_premium_adjusted`, `packs` (addon bundles + plan ladder), and `package_tiers` (comprehensive package tiers with `catalog_id`/`defaults_count`/`addons_count`/`is_current`). `pin_catalog` accepts an optional `catalog_id` to pin a specific catalog directly (package-tier switch).
...
- `PUT /api/business/tiers/{id}` & `DELETE /api/business/tiers/{id}` (edit tier name/key/sort, cascading tier deletion)
- `DELETE /api/business/benefit-concepts/{id}` (retire concept with audit event)
- `PUT /api/business/benefit-aliases/{id}` (edit alias phrase and scope)
- `DELETE /api/business/catalogs/{id}` (retire catalog with audit event)
- `PUT /api/business/catalogs/{catalog_id}/packages/{package_id}/plans/{plan_id}` (rename and reorder plan)
